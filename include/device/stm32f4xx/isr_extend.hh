#ifndef ISR_EXTEND_HH
#define ISR_EXTEND_HH

#include "lib/types.hh"

extern "C" {
    namespace isr {
        struct device_vectors { // Table of pointer to interrupt vectors - STM32F407xC specific interrupts
            void (*WWDG)                (); /*!%0 Window WatchDog             */
            void (*PVD)                 (); /*!%1 PVD through EXTI Line detection */
            void (*TAMP_stamp)          (); /*!%2 Tamper and TimeStamps through the EXTI line */
            void (*RTC_wkup)            (); /*!%3 RTC Wakeup through the EXTI line */
            void (*FLASH)               (); /*!%4 FLASH                       */
            void (*RCC)                 (); /*!%5 RCC                         */
            void (*EXTI0)               (); /*!%6 EXTI Line0                  */
            void (*EXTI1)               (); /*!%7 EXTI Line1                  */
            void (*EXTI2)               (); /*!%8 EXTI Line2                  */
            void (*EXTI3)               (); /*!%9 EXTI Line3                  */
            void (*EXTI4)               (); /*!%10 EXTI Line4                 */
            void (*DMA1_channel1)       (); /*!%11 DMA1 channel 1             */
            void (*DMA1_channel2)       (); /*!%12 DMA1 channel 2             */
            void (*DMA1_channel3)       (); /*!%13 DMA1 channel 3             */
            void (*DMA1_channel4)       (); /*!%14 DMA1 channel 4             */
            void (*DMA1_channel5)       (); /*!%15 DMA1 channel 5             */
            void (*DMA1_channel6)       (); /*!%16 DMA1 channel 6             */
            void (*DMA1_channel7)       (); /*!%17 DMA1 channel 7             */
            void (*ADC)                 (); /*!%18 ADC1, ADC2, ADC3           */
            void (*CAN1_TX)             (); /*!%19 CAN1 TX                    */
            void (*CAN1_RX0)            (); /*!%20 CAN1 RX0                   */
            void (*CAN1_RX1)            (); /*!%21 CAN1 RX1                   */
            void (*CAN1_SCE)            (); /*!%22 CAN1 SCE                   */
            void (*EXTI9_5)             (); /*!%23 External Line[9:5]s        */
            void (*TIM1_BRK_TIM9)       (); /*!%24 TIM1 Break and TIM9        */
            void (*TIM1_UP_TIM10)       (); /*!%25 TIM1 Update and TIM10      */
            void (*TIM1_TRG_COM_TIM11)  (); /*!%26 TIM1 Trigger and Commutation and TIM11 */
            void (*TIM1_CC)             (); /*!%27 TIM1 Capture Compare       */
            void (*TIM2)                (); /*!%28 TIM2                       */
            void (*TIM3)                (); /*!%29 TIM3                       */
            void (*TIM4)                (); /*!%30 TIM4                       */
            void (*I2C1_EV)             (); /*!%31 I2C1 Event                 */
            void (*I2C1_ER)             (); /*!%32 I2C1 Error                 */
            void (*I2C2_EV)             (); /*!%33 I2C2 Event                 */
            void (*I2C2_ER)             (); /*!%34 I2C2 Error                 */
            void (*SPI1)                (); /*!%35 SPI1                       */
            void (*SPI2)                (); /*!%36 SPI2                       */
            void (*USART1)              (); /*!%37 USART1                     */
            void (*USART2)              (); /*!%38 USART2                     */
            void (*USART3)              (); /*!%39 USART3                     */
            void (*EXTI15_10)           (); /*!%40 External Line[15:10]       */
            void (*RTC_Alarm)           (); /*!%41 RTC Alarm (A and B) through EXTI Line */
            void (*OTG_FS_wkup)         (); /*!%42 USB On The Go FS Wakeup through EXTI line */
            void (*TIM8_BRK_TIM12)      (); /*!%43 TIM8 Break and TIM12       */
            void (*TIM8_UP_TIM13)       (); /*!%44 TIM8 Update and TIM13      */
            void (*TIM8_TRG_COM_TIM14)  (); /*!%45 TIM8 Trigger and Commutation and TIM14 */
            void (*TIM8_CC)             (); /*!%46 TIM8 Capture Compare       */
            void (*DMA1_channel8)       (); /*!%47 DMA1 channel 8             */
            void (*FSMC)                (); /*!%48 FSMC                       */
            void (*SDIO)                (); /*!%49 SDIO                       */
            void (*TIM5)                (); /*!%50 TIM5                       */
            void (*SPI3)                (); /*!%51 SPI3                       */
            void (*UART4)               (); /*!%52 UART4                      */
            void (*UART5)               (); /*!%53 UART5                      */
            void (*TIM6_DAC)            (); /*!%54 TIM6 and DAC1&2 underrun errors */
            void (*TIM7)                (); /*!%55 TIM7                       */
            void (*DMA2_channel1)       (); /*!%56 DMA2 channel 1             */
            void (*DMA2_channel2)       (); /*!%57 DMA2 channel 2             */
            void (*DMA2_channel3)       (); /*!%58 DMA2 channel 3             */
            void (*DMA2_channel4)       (); /*!%59 DMA2 channel 4             */
            void (*DMA2_channel5)       (); /*!%60 DMA2 channel 5             */
            void (*ETH)                 (); /*!%61 Ethernet                   */
            void (*ETH_wkup)            (); /*!%62 Ethernet Wakeup through EXTI line */
            void (*CAN2_TX)             (); /*!%63 CAN2 TX                    */
            void (*CAN2_RX0)            (); /*!%64 CAN2 RX0                   */
            void (*CAN2_RX1)            (); /*!%65 CAN2 RX1                   */
            void (*CAN2_SCE)            (); /*!%66 CAN2 SCE                   */
            void (*OTG_FS)              (); /*!%67 On The Go FS               */
            void (*DMA2_channel6)       (); /*!%68 DMA2 channel 6             */
            void (*DMA2_channel7)       (); /*!%69 DMA2 channel 7             */
            void (*DMA2_channel8)       (); /*!%70 DMA2 channel 8             */
            void (*USART6)              (); /*!%71 USART6                     */
            void (*I2C3_EV)             (); /*!%72 I2C3 Event                 */
            void (*I2C3_ER)             (); /*!%73 I2C3 Error                 */
            void (*OTG_HS_EP1_OUT)      (); /*!%74 USB On The Go HS End Point 1 Out */
            void (*OTG_HS_EP1_IN)       (); /*!%75 USB On The Go HS End Point 1 In  */
            void (*OTG_HS_WKUP)         (); /*!%76 USB On The Go HS Wakeup through EXTI */
            void (*OTG_HS)              (); /*!%77 USB On The Go HS           */
            void (*DCMI)                (); /*!%78 DCMI                       */
            void (*CRYP)                (); /*!%79 CRYP crypto                */
            void (*HASH_RNG)            (); /*!%80 Hash and Rng               */
            void (*FPU)                 (); /*!%81 FPU                        */
        };

        void WWDG();
        void PVD();
        void TAMP_stamp();
        void RTC_wkup();
        void FLASH();
        void RCC();
        void EXTI0();
        void EXTI1();
        void EXTI2();
        void EXTI3();
        void EXTI4();
        void DMA1_channel1();
        void DMA1_channel2();
        void DMA1_channel3();
        void DMA1_channel4();
        void DMA1_channel5();
        void DMA1_channel6();
        void DMA1_channel7();
        void ADC();
        void CAN1_TX();
        void CAN1_RX0();
        void CAN1_RX1();
        void CAN1_SCE();
        void EXTI9_5();
        void TIM1_BRK_TIM9();
        void TIM1_UP_TIM10();
        void TIM1_TRG_COM_TIM11();
        void TIM1_CC();
        void TIM2();
        void TIM3();
        void TIM4();
        void I2C1_EV();
        void I2C1_ER();
        void I2C2_EV();
        void I2C2_ER();
        void SPI1();
        void SPI2();
        void USART1();
        void USART2();
        void USART3();
        void EXTI15_10();
        void RTC_Alarm();
        void OTG_FS_wkup();
        void TIM8_BRK_TIM12();
        void TIM8_UP_TIM13();
        void TIM8_TRG_COM_TIM14();
        void TIM8_CC();
        void DMA1_channel8();
        void FSMC();
        void SDIO();
        void TIM5();
        void SPI3();
        void UART4();
        void UART5();
        void TIM6_DAC();
        void TIM7();
        void DMA2_channel1();
        void DMA2_channel2();
        void DMA2_channel3();
        void DMA2_channel4();
        void DMA2_channel5();
        void ETH();
        void ETH_wkup();
        void CAN2_TX();
        void CAN2_RX0();
        void CAN2_RX1();
        void CAN2_SCE();
        void OTG_FS();
        void DMA2_channel6();
        void DMA2_channel7();
        void DMA2_channel8();
        void USART6();
        void I2C3_EV();
        void I2C3_ER();
        void OTG_HS_EP1_OUT();
        void OTG_HS_EP1_IN();
        void OTG_HS_WKUP();
        void OTG_HS();
        void DCMI();
        void CRYP();
        void HASH_RNG();
        void FPU();
    }
}
#endif // ISR_EXTEND_HH
